...
namespace phisi_app {
  ...
  void TextureData::init(uint32_t width, uint32_t height) {
    m_width = width;
    m_height = height;    
  ...
  void TextureData::load_memory(const uint8_t* memory) {
  ...
  uint8_t* TextureData::get_memory() {
    return (uint8_t*) m_mapped_memory;
  }

  void TextureData::flush_memory() {
    vk::MappedMemoryRange range(
      m_staging_memory,
      0, m_image_size);
    checkVkResult(m_device.flushMappedMemoryRanges(1, &range)); 
  ...
  void TextureData::copy_buffer_to_image(vk::CommandBuffer cmd_buffer) {
  ...
  VkDescriptorSet TextureData::get_descriptor() {
